...
# LIVELOCK FIX Phase 4: arms C/D/E on the real model are re-measured with all
...
def _p3cell(ra, arm):
    ...
    rs = [r for r in _p3rows() if r['ratio'] == ra and r['arm'] == arm and r['rc'] == '0']
    if not rs:
        return None
    return (_med([int(r['pager_bytes_fetched']) for r in rs]),
            _med([float(r['tokens_s']) for r in rs]),
            _med([int(r['absent_handled']) for r in rs]),
            _med([float(r['p99_inter_ms']) for r in rs]))
...
def figure6_final():
    """llama.cpp real model, EQUAL BUDGET. bytes read + tokens/s vs budget ratio;
    arms A/C/D/E + OPT. 5 ratios. Arm A: phase1_equal_budget (no pager, unchanged
    by the LIVELOCK FIX). Arms C/D/E: phase3_real_model (all four fixes + the
    A-14 protect_current=off default). Arm E now completes at r=0.25 in its
    default config -- no fallback point."""
    pr = _p1rows(); opt = _p1opt()
    RA = ['0.25','0.375','0.5','0.625','0.75']
    def cellA(ra):
        rs = [r for r in pr if r['ratio']==ra and r['arm']=='A' and r['rc']=='0']
        if not rs: return None
        rb = _med([int(r['io_gen_bytes']) for r in rs if r['io_gen_bytes'] not in ('','n/a')])
    ...
    for arm,(st,col,lab) in style.items():
        ...
        for ra in RA:
            if arm == 'A':
                c = cellA(ra); src = 'phase1_equal_budget'
            else:
                c = _p3cell(ra, arm); src = 'phase3_real_model'
                if c is not None: c = (c[0], c[1], c[2])
            if c is None: continue
            rb,ts,df = c
            xr.append(float(ra)); yb.append(rb/1e9); yt.append(ts)
            ob = opt.get(ra)
            csv_rows.append((ra,arm,f'{rb/1e9:.1f}',f'{rb/ob:.3f}' if ob else '',f'{ts:.2f}',df,'',src))
...
def figure7():
    ...
    for arm,(st,col,lab) in style.items():
        xr,yt = [],[]
        for ra in RA:
            src = 'phase1_equal_budget' if arm == 'A' else 'phase3_real_model'
            pool = pr if arm == 'A' else p3
            rs = [r for r in pool if r['ratio']==ra and r['arm']==arm and r['rc']=='0']
            reps = [float(r['tokens_s']) for r in rs if r['tokens_s'] not in ('','n/a')]
            if not reps: continue
            m = _med(reps)
            xr.append(float(ra)); yt.append(m)
            csv_rows.append((ra,arm,f'{m:.3f}','|'.join(f'{x:.3f}' for x in reps),src))
...
def table1_final():
    """Real-model main results at equal budget. Arm A: phase1 (no pager). Arms
    C/D/E: phase3_real_model (all four LIVELOCK FIX defects + A-14
    protect_current=off). Footnote key inline."""
    pr = _p1rows(); p3 = _p3rows(); opt = _p1opt()
    RA = ['0.25','0.375','0.5','0.625','0.75']
    ...
    for ra in RA:
        ...
        for arm in ('A','C','D','E'):
            ...
            cfg={'A':'kernel mmap, memory.max=B','C':'lru','D':'layer_order_declared, prefetch off','E':'declared + prefetch d2 pinned, protect off'}[arm]
            if arm=='A':
            ...
            if arm=='E' and ra in ('0.25','0.375'):
                note='completes (was a livelock with protect_current on); prefetch non-advantageous vs D here'
...
def table2_final():
    ...
    env = [
        ...
        ('policy (final default)', 'layer_order_declared; consumption signal fires pre-compute (real model) / '
                                   '--consumption-signal all-threads (synthetic); --protect-current OFF on both paths '
                                   '(LIVELOCK FIX A-14). layer_order_learned retained as comparison arm.'),
        ('fetch workers', '4; async dispatch-only handler (A-5)'),
        ('prefetch (arm E)', 'depth 2, retention pinned. Completes at every ratio incl. r <= 0.375 (LIVELOCK FIX); '
                             'prefetch remains non-advantageous vs arm D at r <= 0.375 -- run arm D there.'),
...
import sys as _sys
_ALL = {'figure1':figure1,'figure2':figure2,'figure3':figure3,'figure4':figure4,
        'figure5':figure5,'figure6':figure6,'table1':table1,'table2':table2,
        'figure6_final':figure6_final,'figure7':figure7,'table1_final':table1_final,
        'table2_final':table2_final}
_sel = _sys.argv[1:]
if _sel:
    for _n in _sel: _ALL[_n]()
else:
