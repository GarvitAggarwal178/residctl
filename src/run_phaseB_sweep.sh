#!/bin/bash
# Campaign 12 Phase B: chunk-size floor. Region fixed 2 GiB.
# --chunk-size in {4,8,16,32}MiB (512,256,128,64 chunks).
# Arms C, D, E. 3 ratios, n=3, async, --fetch-workers 4 --driver-threads 8
# --lookahead-window 1 --prefetch-depth 2 --prefetch-retention pinned
# --compute-ns-per-mib 400000.
# Arm A (single fixed 'sequential' mode, matching Phase 3's own disclosed
# simplification) + arm B, using Phase A's repaired drop_caches pattern
# and guard-equipped baseline_main.
set -u
...
CGROUP=/sys/fs/cgroup/residctl_phaseB
CSV=$RESULTS/campaign12_phaseB_chunk_floor.csv
LOG=$RESULTS/campaign12_phaseB_chunk_floor_log.txt
...
if [ ! -f "$CSV" ]; then
    > "$LOG"
    > "$CSV"
    echo "chunk_size,ratio,arm,rep,touches,bytes_touched,wall_ns,absent_handled,evictions,infeasible,prefetches,pager_bytes_fetched,io_read_bytes_delta,dedup_resident,dedup_fetching,pin_broken,rc" > "$CSV"
    log "=== fresh start ==="
else
...
rep_done() {
    local cs=$1 ratio=$2 arm=$3 rep=$4
    awk -F, -v c="$cs" -v r="$ratio" -v a="$arm" -v rp="$rep" \
        'NR>1 && $1==c && $2==r && $3==a && $4==rp {found=1} END{exit !found}' "$CSV"
}
...
run_row() {
    ...
    echo "$cs,$ratio,$arm,$rep,${touches:-},${bytes_touched:-},${wall_ns:-},${absent:-},${evictions:-},${infeasible:-},${prefetches:-},${pbf:-},${iord:-},${dedr:-},${dedf:-},${pinb:-},$rc" >> "$CSV"
    cleanup_cgroup
}
...
run_cell() {
    ...
    for rep in 1 2 3; do
        if rep_done "$cs" "$ratio" "$arm" "$rep"; then
            log "SKIP (rep already recorded): chunk_size=$cs ratio=$ratio arm=$arm rep=$rep"
            continue
        fi
        ...
        case "$arm" in
            ...
            B)
                ...
                ;;
            C)
                trace="$SCRATCH/phaseB_C_cs${cs}_r${ratio}_rep${rep}.fetchtrace"
                rm -f "$trace"
                run_row "$cs" "$ratio" "$arm" "$rep" \
                ...
                ;;
            D)
                trace="$SCRATCH/phaseB_D_cs${cs}_r${ratio}_rep${rep}.fetchtrace"
                reftrace="$SCRATCH/phaseB_D_cs${cs}_r${ratio}_rep${rep}.reftrace"
                rm -f "$trace" "$reftrace"
                run_row "$cs" "$ratio" "$arm" "$rep" \
                    "$SRC/replay_main" "$CGROUP" "$MODEL" "$REGION_LEN" "$cs" "$budget_bytes" "$N_PASSES" \
                ...
                ;;
            E)
                trace="$SCRATCH/phaseB_E_cs${cs}_r${ratio}_rep${rep}.fetchtrace"
                rm -f "$trace"
                run_row "$cs" "$ratio" "$arm" "$rep" \
                    "$SRC/replay_main" "$CGROUP" "$MODEL" "$REGION_LEN" "$cs" "$budget_bytes" "$N_PASSES" \
...
ps aux --sort=-%cpu 2>/dev/null | head -8 | tee -a "$LOG"
log ""
log "=== Phase B sweep complete (or resumed to completion), results in $CSV ==="
