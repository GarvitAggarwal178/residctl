#!/bin/bash
CSV=/root/residctl/results/final/phase3_arm_e.csv
...
while :; do
    ...
    if ! pgrep -f run_final_phase3.sh >/dev/null; then
        ...
        cd /root/residctl && setsid bash src/run_final_phase3.sh >> results/final/phase3_console.txt 2>&1 < /dev/null & disown
        sleep 5
    else
