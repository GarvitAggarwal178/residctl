#!/bin/bash
# item 10d Task B: sync vs async under REAL concurrency. Arm D (layer_order,
# prefetch off), V2 parameters (2 GiB region, 128 MiB chunks, 5 passes), 3
# budget ratios x n=3 x --driver-threads {1,2,4,8} x handler {sync, async
# --fetch-workers 4}. --fetch-trace captured per run for device-busy
# fraction (union-of-read-intervals, item 10b/10c method) and median
# dispatch latency (entry to enqueue, item 10d's new fetch_trace fields).
set -u
...
CGROUP=/sys/fs/cgroup/residctl_sweep_d_b
CSV=$RESULTS/task_d_sweep_b.csv
LOG=$RESULTS/task_d_sweep_b_log.txt
> "$LOG"
> "$CSV"
echo "ratio,driver_threads,handler,rep,touches,bytes_touched,wall_ns,absent_handled,evictions,infeasible,dedup_resident,dedup_fetching,pager_bytes_fetched,io_read_bytes_delta,censored,exit_code" > "$CSV"
...
run_one() {
    local ratio=$1 threads=$2 handler=$3 rep=$4 budget_bytes=$5; shift 5
    local out rc censored=0
    out=$(bash -c 'echo $BASHPID > "'"$CGROUP"'/cgroup.procs"; exec timeout 60 "$@"' -- "$@" 2>&1)
    rc=$?
    log "--- ratio=$ratio threads=$threads handler=$handler rep=$rep rc=$rc ---"
    log "$out"
    if [ $rc -eq 137 ]; then
        censored=1
        log "CENSORED: OOM-killed at ratio=$ratio threads=$threads handler=$handler rep=$rep"
    fi
    ...
    echo "$ratio,$threads,$handler,$rep,${touches:-},${bytes_touched:-},${wall_ns:-},${absent:-},${evictions:-},${infeasible:-},${dedr:-},${dedf:-},${pbf:-},${iord:-},$censored,$rc" >> "$CSV"
    cleanup_cgroup
}
...
for ratio in $RATIOS; do
    ...
    for threads in $THREAD_COUNTS; do
        for rep in 1 2 3; do
            trace="$SCRATCH/sweepb_sync_t${threads}_r${ratio}_rep${rep}.fetchtrace"
            rm -f "$trace"
            fresh_cgroup "$memmax"
            run_one "$ratio" "$threads" "sync" "$rep" "$budget_bytes" \
                "$SRC/replay_main" "$CGROUP" "$MODEL" "$REGION_LEN" "$CHUNK_SIZE" "$budget_bytes" "$N_PASSES" layer_order off --fetch-trace "$trace" --sync-handler --driver-threads "$threads"
        done
        for rep in 1 2 3; do
            trace="$SCRATCH/sweepb_async_t${threads}_r${ratio}_rep${rep}.fetchtrace"
            rm -f "$trace"
            fresh_cgroup "$memmax"
            run_one "$ratio" "$threads" "async" "$rep" "$budget_bytes" \
                "$SRC/replay_main" "$CGROUP" "$MODEL" "$REGION_LEN" "$CHUNK_SIZE" "$budget_bytes" "$N_PASSES" layer_order off --fetch-trace "$trace" --fetch-workers 4 --driver-threads "$threads"
        done
    done
done
...
free -h | tee -a "$LOG"
log ""
log "=== Sweep B complete, results in $CSV ==="
