#!/bin/bash
# Build-order item 4 verification: eviction + budget + reconcile (§7).
set -u
RESIDCTL=/root/residctl
BIN=$RESIDCTL/src/test_eviction
GEN=$RESIDCTL/src/gen_pattern
MODEL=$RESIDCTL/scratch/pattern_16m.bin
RESULTS=$RESIDCTL/results
CGROUP=/sys/fs/cgroup/residctl_item4
OUT=$RESULTS/item4_test_log.txt
> "$OUT"
