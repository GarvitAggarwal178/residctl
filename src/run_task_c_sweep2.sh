#!/bin/bash
# item 10c Task C, Sweep 2: full 6-arm sweep under the item 10c defaults --
# async handler, --fetch-workers 4, --prefetch-admission guarded,
# --prefetch-depth 2 (item 10b's best depth). Same table structure as
# HARNESS_REPORT_V2.md's sweep (run_item10_v2_harness.sh), same V2
# parameters (2 GiB region, 128 MiB chunks, 5 passes), 3 ratios, n=3/cell.
# Primary metric: read_bytes. drop_caches before every arm A/B invocation
# (arms C/D/E use O_DIRECT, bypassing page cache -- no drop_caches needed,
# matching the V2 harness's own convention).
set -u
...
> "$LOG"
> "$CSV"
echo "ratio,arm,detail,rep,touches,bytes_touched,wall_ns,absent_handled,evictions,infeasible,prefetches,pager_bytes_fetched,io_read_bytes_delta,dedup_resident,dedup_fetching,prefetch_declined,censored,exit_code" > "$CSV"
...
for ratio in $RATIOS; do
    ...
    for rep in 1 2 3; do
        ...
        run_arm "$ratio" "C" "lru_off" "$rep" \
            "$SRC/replay_main" "$CGROUP" "$MODEL" "$REGION_LEN" "$CHUNK_SIZE" "$budget_bytes" "$N_PASSES" lru off "" "" --fetch-workers 4 --prefetch-depth 2 --prefetch-admission guarded > /dev/null
    done

    # --- Arm D: pager, layer_order, prefetch off, n=3 (reference trace kept for OPT) ---
    for rep in 1 2 3; do
        d_trace="$SCRATCH/sweep2_arm_d_ratio_${ratio}_rep${rep}.reftrace"
        rm -f "$d_trace"
        fresh_cgroup "$memmax"
        run_arm "$ratio" "D" "layer_order_off" "$rep" \
            "$SRC/replay_main" "$CGROUP" "$MODEL" "$REGION_LEN" "$CHUNK_SIZE" "$budget_bytes" "$N_PASSES" layer_order off "" "$d_trace" --fetch-workers 4 --prefetch-depth 2 --prefetch-admission guarded > /dev/null
    done
    ...
    # --- OPT: offline solver over EACH of D's 3 reference traces ---
    for rep in 1 2 3; do
        d_trace="$SCRATCH/sweep2_arm_d_ratio_${ratio}_rep${rep}.reftrace"
        if [ -s "$d_trace" ]; then
...
free -h | tee -a "$LOG"
log ""
log "=== Sweep 2 complete, results in $CSV ==="
