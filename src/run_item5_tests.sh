#!/bin/bash
# Build-order item 5 verification: trace recorder + metrics (§9).
set -u
RESIDCTL=/root/residctl
BIN=$RESIDCTL/src/test_trace
GEN=$RESIDCTL/src/gen_pattern
MODEL=$RESIDCTL/scratch/pattern_16m.bin
TRACE=$RESIDCTL/scratch/item5_trace.bin
RESULTS=$RESIDCTL/results
CGROUP=/sys/fs/cgroup/residctl_item5
OUT=$RESULTS/item5_test_log.txt
> "$OUT"
